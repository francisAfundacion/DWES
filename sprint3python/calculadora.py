...
while otra_operacion == "s":
    resultado = 0
    numero_1 = int(input('Introduzca primer número para la operación aritmética deseada: ').strip())
    numero_2 = int(input('Introduzca segundo número para la operación aritmética deseada: ').strip()) #limpiar el intro
    tipo_operacion = input('Introduzca tipo de operación que se desea realizar(suma, resta, multiplicacion, division): ').strip()
    ...
    otra_operacion = input("¿Desea realizar otra operación?s(para efectuar otra operación)n(para finalizar): ").strip() #limpiar el intro
